...
if __name__ == '__main__':
    # Default to Gerrit
    default_gerrit = 'https://review.wrathos.com'
    ...
    for project in projects:
        name = project.get('name').replace("WrathOS/", "")
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit wrath', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch wrath', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or wrath failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
